...
Refer to [Challenge Readme](./challenge-docs/Readme.md) for a detailed explanation of the solution, including architecture, API endpoints, and testing.

> **Note on concurrency:** The current architecture does not support running multiple workers concurrently. The background worker is started once and runs as a single loop. Multi-worker support was not implemented because it was not explicitly requested by the challenge.
